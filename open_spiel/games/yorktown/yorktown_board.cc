...
namespace open_spiel
{
  namespace yorktown
  {
    ...
    std::string Piece::ToString(Color c) const
    {
      ...
      if (c == Color::kEmpty)
      {
        if (color == Color::kBlue)
          return isVisible ? "b" + base : "B" + base;
        else
          return isVisible ? "r" + base : "R" + base;
      }
      else if (c == Color::kBlue)
      {
        if (color == Color::kBlue)
          ...
          return isVisible ? "r" + base : "R?";
      }
      else if (c == Color::kRed)
      {
        if (color == Color::kBlue)
          return isVisible ? "b" + base : "B?";
        else
          return isVisible ? "r" + base : "R" + base;
      }
    }
    ...
    {
      if (s.size() != 2)
        return chess_common::kInvalidSquare;
    ...
    std::string Move::ToComplexLANMove(const YorktownBoard<10> &board) const
    {
      ...
      if (board.at(from).isVisible && board.at(from).type != PieceType::kEmpty)
        absl::StrAppend(&ret, "[", PieceTypeToString(board.at(from).type), "]");
      if (board.at(to).type == PieceType::kEmpty)
        absl::StrAppend(&ret, "-");
      else
        absl::StrAppend(&ret, "x");
      absl::StrAppend(&ret, SquareToStrategoString(to));
      if (board.at(to).isVisible && board.at(to).type != PieceType::kEmpty)
        absl::StrAppend(&ret, "[", PieceTypeToString(board.at(to).type), "]");
      return ret;
    }
    ...
    YorktownBoard<kBoardSize>::BoardFromStraDos3(const std::string &straDos)
    {
      ...
      for (int8_t current_y = 0; current_y < kBoardSize; current_y++)
      {
        for (int8_t current_x = 0; current_x < kBoardSize; current_x++)
        {
          ...
          Color color;
          if ((piece_on_sq > 'A' && piece_on_sq < 'N') || (piece_on_sq > 'a' && piece_on_sq < 'n'))
            color = Color::kRed;
          else if ((piece_on_sq >= 'N' && piece_on_sq < 'Z') || (piece_on_sq >= 'n' && piece_on_sq < 'z'))
            color = Color::kBlue;
          else
            color = Color::kEmpty;
          ...
          bool visible = true;
          ;
          if (piece_on_sq > 'A' && piece_on_sq <= 'Z')
            visible = false;
...
#undef YIELD
        ...
        const std::string &move) const
    {
      ...
      if (move.size() == 8)
      {
        std::string LAN;
        if (move.substr(3, 1) == "[")
          LAN = move.substr(0, 2) + move.substr(6, 2);
        else
          LAN = move.substr(0, 2) + move.substr(3, 2);
        return ParseLANMove(LAN);
      }
    ...
    void YorktownBoard<kBoardSize>::ApplyMove(const Move &move)
    {
      ...
      {
        if (abs(move.to.x - move.from.x) + abs(move.to.y - move.from.y) > 1)
        {
          assert(move.piece.type == PieceType::kScout);
    ...
    std::string YorktownBoard<kBoardSize>::DebugString(Color c, bool onlyBoard) const
    {
      std::string s;
      if (onlyBoard == false)
        s = absl::StrCat("Strados: ", ToStraDos3(c), "\n");
      ...
      if (c == Color::kRed)
        absl::StrAppend(&s, "This is the partial observation of the red player \n");
      else if (c == Color::kBlue)
        absl::StrAppend(&s, "This is the partial observation of the blue player \n");
      else
        absl::StrAppend(&s, "This is the observers view with all information \n");
