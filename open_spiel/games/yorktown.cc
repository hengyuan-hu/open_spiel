...
inline float PROTECT_FROM_NAN(const float x)
{
  if (std::isnan(x))
  {
    return 0.0;
...
namespace open_spiel
{
  namespace yorktown
  {
              ...
              if (piece_on_board.hasMoved == true)
              {
                if (piece_type != PieceType::kFlag && piece_type != PieceType::kBomb)
                {
                  for (int i = 0; i < 12; ++i)
                  {
                    ...
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i] - board.find(Piece{color, piece_type, true}).size()) / ((float)countMovableInvisible));
                    }
                    if (piece_type == kPieceTypes[i] && color == Color::kBlue)
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i + 12] - board.find(Piece{color, piece_type, true}).size()) / ((float)countMovableInvisible));
                    }
                  }
                }
                else
              ...
              else
              {
                for (int i = 0; i < 12; ++i)
                {
                  if (piece_type == PieceType::kFlag || piece_type == PieceType::kBomb)
                  {
                    if (piece_type == kPieceTypes[i] && color == Color::kRed)
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i] - board.find(Piece{color, piece_type, true}).size()) / ((float)countNotMovedInvisbible));
                    }
                    if (piece_type == kPieceTypes[i] && color == Color::kBlue)
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i + 12] - board.find(Piece{color, piece_type, true}).size()) / ((float)countNotMovedInvisbible));
                    }
                  }
                  else
                  {
                    if (piece_type == kPieceTypes[i] && color == Color::kRed)
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i] - board.find(Piece{color, piece_type, true}).size()) * (pMovableGivenNotMovedInvisible) / ((float)countMovableInvisible));
                    }
                    if (piece_type == kPieceTypes[i] && color == Color::kBlue)
                    {
                      *value_it++ = PROTECT_FROM_NAN(((float)board.LivingPieces()[i + 12] - board.find(Piece{color, piece_type, true}).size()) * (pMovableGivenNotMovedInvisible) / ((float)countMovableInvisible));
                    }
                  }
                }
              }
    ...
    std::string YorktownState::ToString() const
    {
